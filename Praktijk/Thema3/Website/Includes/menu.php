<?php
/*
 * Naam: Tara van Litsenburg
 * Datum: 15-03-2022
 * menu.php
*/
?>
    <!--Css Koppeling-->
    <link href="../../Website/Styles/nav.css" rel="stylesheet">
    <!--Java script koppeling-->
    <script src="../Scripts/faq.js" defer></script>
    <!--Navigatie code-->
    <ul>
        <li><a href="/Praktijk/Thema3/Website/index.php">Home</a></li>
        <li><a href="/Praktijk/Thema3/Website/Pages/sport.php">Sport</a></li>
        <li><a href="/Praktijk/Thema3/Website/Pages/lesrooster.php">Lesrooster</a></li>
        <li><a href="/Praktijk/Thema3/Website/Pages/personaltrainer.php">Personal Trainer</a></li>
        <li><a href="/Praktijk/Thema3/Website/Pages/overons.php">Over ons</a></li>
        <li><a href="/Praktijk/Thema3/Website/Pages/lidworden.php">Lid worden</a></li>
        <li><a href="/Praktijk/Thema3/Website/Pages/zakelijk.php">Zakelijk</a></li>
        <li><a href="/Praktijk/Thema3/Website/Pages/contact.php">Contact</a></li>
        <li><a href="/Praktijk/Thema3/Website/Pages/faq.php">FAQ</a></li>
    </ul>
